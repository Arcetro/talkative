...
fleetRouter.post("/fleet/nodes", async (req, res) => {
  try {
    ...
    await upsertTenant({ tenant_id, name: tenant_id });
...
fleetRouter.post("/fleet/agents", async (req, res) => {
  try {
    ...
    await upsertTenant({ tenant_id, name: tenant_id });
    await upsertCloud({ tenant_id, name: cloud_id, provider: "local", region: "local" });

    const agent = await agentHub.createAgent({ id, tenant_id, name, workspace: id ?? undefined });
    for (const skill of skills) {
      await agentHub.attachSkill(agent.id, { skillName: skill });
    }
...
fleetRouter.post("/fleet/agents/:id/provision", async (req, res) => {
  try {
    ...
    const agent = agentHub.getAgent(req.params.id);
    ...
    return res.json({ ok: true, tenant_id, agent_id: agent.agent_id, node_id, ...result });
  } catch (error) {
    return res.status(400).json({ error: (error as Error).message });
  }
});
