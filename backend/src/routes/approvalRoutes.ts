...
approvalRouter.post("/approvals", async (req, res) => {
  try {
    ...
    return res.status(201).json(await createApproval({ tenant_id, agent_id, run_id, reason }));
  } catch (error) {
...
approvalRouter.post("/approvals/:id/decision", async (req, res) => {
  try {
    ...
    return res.json(await decideApproval({ id: req.params.id, operator_id, decision, note }));
  } catch (error) {
    return res.status(400).json({ error: (error as Error).message });
  }
});
